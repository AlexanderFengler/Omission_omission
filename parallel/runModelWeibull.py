...
alpha_list  = np.linspace(1,3,3)
beta_list = np.linspace(1,1.5,3)
a_index = jobid//9
sub_id = jobid%9
...
for i in range(30):
    ...
    sim_out = ssms.basic_simulators.simulator.simulator(
        model='weibull', theta=[v, a, z, t, alpha,beta], n_samples=nsample
    )
    rt_twoside = np.random.uniform(-5,5,nsample)
    choice = 2 * (rt_twoside >= 0) - 1
    rt = np.abs(rt_twoside)
    outlier = np.zeros((nsample,2))
    outlier[:,0] = rt
    outlier[:,1] = choice
    arr = np.arange(0,nsample,1)
    ...
    with pm.Model() as ddm:
        ...
        a1 = pm.Uniform("a", 0.5, 1.5)
        z1 = pt.constant(0.5)
        t1 = pm.Uniform("t", 0.1, 0.5)
        alpha1 =pm.Uniform("alpha", 0.5, 4.5)
        beta1  =pm.Uniform("beta", 0.5, 4.5)
        deadline = pt.constant(1.25)
        p_outlier1 = pm.Uniform("p_outlier",-10,10)
        # t = pm.Uniform("t", 0.0, 2.0)
        # theta = pm.Uniform("theta",0,1.3)
        # ddl = pt.constant(1.25)
        
    
        pm.Potential("choice_rt", lan_logp_op(data_commission, v1, a1, z, t, alpha1,beta1, p_outlier1))
        ...
        ddm_blog_traces_numpyro_nd = pmj.sample_numpyro_nuts(
                chains=2, draws=1000, tune=5000,target_accept=0.99,progressbar=False)
        post_summary = pm.summary(ddm_blog_traces_numpyro_nd)                 

    v_pred_nd.append(post_summary.loc['v','mean'])
    a_pred_nd.append(post_summary.loc['a','mean'])
    alpha_pred_nd.append(post_summary.loc['alpha','mean'])
    beta_pred_nd.append(post_summary.loc['beta','mean'])
    p_pred_nd.append(1/(1+np.exp(-post_summary.loc['p_outlier','mean']))*0.2)
    ...
    with pm.Model() as ddm:
        ...
        pm.Potential("choice_rt", lan_logp_op(data_commission, v1, a1, z, t, alpha1,beta1,p_outlier1))
        pm.Potential("omission", cpn_logp_op(data_omission, v1, a1, z, t, alpha1,beta1,deadline,p_outlier1))

        ddm_blog_traces_numpyro = pmj.sample_numpyro_nuts(
                chains=2, draws=1000, tune=5000,target_accept=0.99,progressbar=False)
        post_summary = pm.summary(ddm_blog_traces_numpyro)

    v_pred_d.append(post_summary.loc['v','mean'])
    a_pred_d.append(post_summary.loc['a','mean'])
    alpha_pred_d.append(post_summary.loc['alpha','mean'])
    beta_pred_d.append(post_summary.loc['beta','mean'])
    p_pred_d.append(1/(1+np.exp(-post_summary.loc['p_outlier','mean']))*0.1)
...
percentile_list.to_csv('results_weibull/p001_3/output_'+sys.argv[1]+'.csv',index=False)
